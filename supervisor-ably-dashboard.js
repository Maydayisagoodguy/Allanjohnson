...
if (ablyReady) {
  const client = new Ably.Realtime({ key: ablyConfig.apiKey, clientId: "supervisor-dashboard" });
  const channel = client.channels.get(ablyConfig.channelName || "mn-moopen-geofence-demo");
  channel.subscribe("location-update", (message) => {
    caState.set(message.data.caId, message.data);
    render([...caState.values()]);
  });
} else {
  document.querySelector("#alertList").innerHTML =
    '<div class="alert">Ably is not configured. Paste your Ably API key in ably-config.js.</div>';
}
